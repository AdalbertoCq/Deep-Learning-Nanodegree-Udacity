...
class Actor:
    def __init__(self, state_space, action_space, action_min, action_max, hidden_units, learning_rate):
        ...
        var_wi = initializers.lecun_normal()
        input_states = layers.Input(shape=(self.state_space,), name='input_states')
        
        layer = layers.Dense(units=2*hidden_units, kernel_initializer=var_wi, activation='elu')(input_states)
        layer = layers.Dense(units=hidden_units, kernel_initializer=var_wi, activation='elu')(layer)
        
        norm_action = layers.Dense(self.action_space, kernel_initializer=var_wi, activation='sigmoid', name='norm_action')(layer)
...
class Critic:
    def __init__(self, state_space, action_space, hidden_units, learning_rate):
        ...
        # States network.
        input_states = layers.Input(shape=(self.state_space,), name='input_states')
        layer_states = layers.Dense(units=2*hidden_units, kernel_initializer=var_wi, activation='elu')(input_states)

        # Actions network.
        input_actions = layers.Input(shape=(self.action_space,), name='input_actions') 
        layer_actions = layers.Dense(units=2*hidden_units, kernel_initializer=var_wi, activation='elu')(input_actions)
        ...
        q_values = layers.Dense(units=1, kernel_initializer=var_wi, name='q_values')(layer)
...
class DDPG_Agent:
    def __init__(self, task, noise, memory, rl_param, nn_hidden, actor_lr, critic_lr, q_lambda):
        ...
        self.actor_target = Actor(self.state_space, self.action_space, self.action_low, self.action_high,
                                  hidden_units=nn_hidden[0], learning_rate=actor_lr)
